...
| POST | `/expenses/scan` | Mock OCR (REWE GmbH 47.80€) |
...
| product-014 | POST `/expenses/scan` | Mock OCR |
...
| `/my-expenses/create/scan` | `scan-view.tsx` | Upload + Mock OCR split view |
...
| ---- | ----------- | ---- |
| RECEIPT | Beleg | Upload + Mock OCR extraction |
...
| BFF Products | product-018+ cho finance endpoints |
...
| **BFF pattern** | Mỗi product = `controller.ts` + `index.ts`, đánh số product-001 → product-017 |
| **RTK Query** | `authApi` (auth/onboarding) + `expenseApi` (expense/manager) |
| **i18n** | de-DE, en-US, vi-VN |
| **Mock OCR** | `ExpenseService.mockScan()` — thay bằng real OCR khi cần |
| **Route ordering** | BFF: `/expenses/scan` phải register trước `/expenses/:id` |
