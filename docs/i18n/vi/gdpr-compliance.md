...
sequenceDiagram
    ...
    Note over ADM: Review data map<br/>Phân loại: PII vs Financial
...
Frontend derives status by checking which follow-up events exist for the same `subject_token`:
...
timeline
    ...
    section Medium — GDPR minimization
      ...
      user_consents        : ≥3 years after withdraw
...
**Cross-phase audit trail cho 1 erasure request** — 5 events linked by `subject_token = SHA256(cognito_sub)`:
